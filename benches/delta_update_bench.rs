...
pub fn criterion_benchmark(c: &mut Criterion) {
    ...
    // Run once to validate output as expected
    output_playlist = vec![];
    make_delta_update_using_hls_m3u8(LONG_MEDIA_PLAYLIST.as_bytes(), &mut output_playlist)
        .expect("should not fail");
    assert_eq!(
        KIND_OF_EXPECTED_HLS_M3U8_OUTPUT,
        std::str::from_utf8(&output_playlist.clone()).expect("output should be valid string")
    );
    // Then run the bench
    c.bench_function(
        ...
        |b| {
            b.iter(|| {
                output_playlist = vec![];
                black_box(
                    make_delta_update_using_hls_m3u8(
                        LONG_MEDIA_PLAYLIST.as_bytes(),
                        &mut output_playlist,
                    )
                    .expect("should not fail"),
...
fn make_delta_update<W: Write>(input: &[u8], output: &mut W) -> Result<(), Box<dyn Error>> {
    ...
    // We need to write out how many segments we've skipped (only if we have removed any).
...
// =================================================================================================
//
// Delta update implementation using hls_m3u8
//
// hls_m3u8 suffers from all of the same issues that m3u8-rs does, but also has additional problems
// that make it impossible to implement delta updates properly:
//   * There is no control over the EXT-X-VERSION tag. The library determines this itself based on
//     what it knows from the tags that have been included; however, the library is out of date with
//     respect to the HLS specification, and so it outputs an incorrect version tag.
//   * The library decides that it must have EXT-X-KEY tag on every segment, regardless of the
//     source manifest, and will put EXT-X-KEY tags everywhere. Interestingly, since we removed the
//     key tag with the delta update, the library interprets this to mean that there is no DRM, and
//     puts key tags with METHOD=NONE everywhere. This is quite broken.
...
//     recognize.
//
// For me I haven't found it possible to implement a delta update using this library.
//
// =================================================================================================

fn make_delta_update_using_hls_m3u8<W: Write>(
    input: &[u8],
    output: &mut W,
) -> Result<(), Box<dyn Error>> {
    let input_str = std::str::from_utf8(input)?;
    let mut media_playlist = MediaPlaylist::try_from(input_str)?;

    let skip_until = 6.0 * media_playlist.target_duration.as_secs_f32();
    // Interestingly, we can't set the version using hls_m3u8, as the library doesn't expose this.
    // Instead, hls_m3u8 determines what the version should be based on the tags it knows. This
    // seems a little short-sighted to me, as it requires that the library is at least as up to date
    // as the latest HLS specification to be able to determine where new versions are needed. Case
    // in point, it doesn't look like the library supports EXT-X-SKIP, and so won't be able to set
    // the version high enough. In fact, we can find that the maximum supported HLS version of the
    // library is v7, based on `src/types/protocol_version.rs`.
    let mut existing_server_control: Option<Cow<str>> = None;
    // hls_m3u8 does not support the EXT-X-SERVER-CONTROL tag. This means I have to work with
    // Cow<str> directly via the `unknown` vec. hls_m3u8 does write out unknown tags for the media
    // playlist; however, similar to m3u8-rs, it has an opinionated order of how tags are written
    // and chooses to put unknown tags after all segments (but before any endlist). This means that
    // we can't use these unknown tags for writing using the library.
    for tag in &media_playlist.unknown {
        if tag.starts_with("#EXT-X-SERVER-CONTROL") {
            if tag.contains("CAN-SKIP-UNTIL") {
                // This will get complicated here. In this scenario I would need to custom parse the
                // server control tag, extract the CAN-SKIP-UNTIL value, and then update the skip
                // until we are working with here. Since I know this isn't the case for the bench,
                // I'll avoid going through this complexity, but the implementation is therefore
                // incomplete.
                todo!()
            } else {
                existing_server_control =
                    Some(Cow::Owned(format!("{tag},CAN-SKIP-UNTIL={skip_until}")));
            }
            break;
        }
    }
    // Given that hls_m3u8 just dumps all unknown media playlist lines to the bottom when writing,
    // it's just better to remove all of them, after we've found our server control if it exists.
    media_playlist.unknown.clear();
    // hls_m3u8 doesn't support the concept of EXT-X-SKIP. Furthermore, any tag that can appear
    // within the segments section (e.g. EXT-X-DATERANGE) is tied to a segment, which means I can't
    // include them without having segments. This is problematic because I need to leave the skipped
    // over EXT-X-DATERANGE in the playlist, so I have to write these to intermediate data, and then
    // stitch it together after writing. I'll start with the server control tag and then later add
    ...
    // For hls_m3u8 we need to do things a little differently, because unlike m3u8-rs (or our lib),
    // hls_m3u8 uses the stable_vec crate to hold its segments, which does not have a drain method.
    // Instead, we can retain what we want, and move info from removed segments prior to removing
    // them.
    let total_segment_count = media_playlist.segments.num_elements();
    let mut retained_indices = vec![];
    let mut backwards_segment_duration = 0.0;
    for (index, segment) in media_playlist.segments.iter().rev() {
        if backwards_segment_duration >= skip_until {
            break;
        } else {
            retained_indices.push(index);
            backwards_segment_duration += segment.duration.duration().as_secs_f32();
        }
    }
    // Now we have to loop forward to save information from the segments that are about to be
    // removed.
    for (i, segment) in &media_playlist.segments {
        if retained_indices.contains(&i) {
            continue;
        }
        // Perhaps even more strange, hls_m3u8 has the same bug as m3u8-rs, in that it only accounts
        // for the possibility of one daterange being present for a segment at a time. Perhaps there
        // was some sharing of ideas here? In any case, like before, this makes it very difficult to
        // use the library effectively, as we will lose a lot of daterange markers.
        if let Some(daterange) = &segment.date_range {
            writeln!(intermediate_lines, "{daterange}")?;
        }
    }
    media_playlist
        .segments
        .retain_indices(|i| retained_indices.contains(&i));
    writeln!(
        intermediate_lines,
        "#EXT-X-SKIP:SKIPPED-SEGMENTS={}",
        total_segment_count - retained_indices.len()
    )?;
    let mut temporary_output = Vec::new();
    writeln!(temporary_output, "{media_playlist}")?;
    // Here I take a fairly dumb approach to finding the first media segment tag by looping through
    // the bytes until I find the name of a media segment tag.
    let mut last_tag_token_index = 0;
    let mut insertion_index = None;
...
const KIND_OF_EXPECTED_HLS_M3U8_OUTPUT: &str = r#"#EXTM3U
#EXT-X-VERSION:3
#EXT-X-TARGETDURATION:4
#EXT-X-MEDIA-SEQUENCE:541647
#EXT-X-SERVER-CONTROL:CAN-SKIP-UNTIL=24.0
